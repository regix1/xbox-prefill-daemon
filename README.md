...
The daemon keeps one persistent Microsoft account/device session while accepting
independent prefill runs. Each run processes its products in order;
different runs may transfer disjoint products concurrently. It never creates
another account session or container for a schedule.
