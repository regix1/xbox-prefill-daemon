...
VERSION_API="https://api.github.com/repos/regix1/xbox-prefill-daemon/releases"
...
# Checking to see if XboxPrefill is already up to date
if [ -f ./XboxPrefill ]; then
    CURRENT_VERSION=$(./XboxPrefill --version)
...
DOWNLOAD_URL="https://github.com/regix1/xbox-prefill-daemon/releases/download/v${LATEST_TAG}/XboxPrefill-${LATEST_TAG}-linux-x64.zip"
wget -q -nc --show-progress --progress=bar:force:noscroll $DOWNLOAD_URL

# Unzip
echo -e "${Yellow} Unzipping... ${NC}"
unzip -q -j -o XboxPrefill-${LATEST_TAG}-linux-x64.zip

# Required so executable permissions don't get overwritten by unzip
chmod +x XboxPrefill update.sh

# Cleanup
rm XboxPrefill-${LATEST_TAG}-linux-x64.zip
