...
[xml]$parsedCsproj = Get-Content .\XboxPrefill\XboxPrefill.csproj
...
$currentContent = Get-Content -Path .\XboxPrefill\XboxPrefill.csproj -Raw
$currentContent = $currentContent.Replace('<VersionPrefix>' + $currentVersion, '<VersionPrefix>' + $newVersion)
Set-Content -Value $currentContent -Path .\XboxPrefill\XboxPrefill.csproj -NoNewline
